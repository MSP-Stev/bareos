...
static ResourceItem dir_items[] = {
  ...
  { "PidDirectory", CFG_TYPE_DIR, ITEM(res_dir, pid_directory), 0, CFG_ITEM_DEFAULT | CFG_ITEM_PLATFORM_SPECIFIC, PATH_BAREOS_PIDDIR, NULL, NULL },
    ...
    bool verbose)
{
  ...
  send.KeyMultipleStringsOnePerLine("Regex", fo->regex);
  send.KeyMultipleStringsOnePerLine("RegexDir", fo->regexdir);
  send.KeyMultipleStringsOnePerLine("RegexFile", fo->regexfile);
  send.KeyMultipleStringsOnePerLine("Wild", fo->wild);
  send.KeyMultipleStringsOnePerLine("WildDir", fo->wilddir);
  send.KeyMultipleStringsOnePerLine("WildFile", fo->wildfile);
  /*
   ...
   */
  send.KeyMultipleStringsOnePerLine("WildBase", fo->wildbase);
  send.KeyMultipleStringsOnePerLine("Base", fo->base);
  send.KeyMultipleStringsOnePerLine("FsType", fo->fstype);
  send.KeyMultipleStringsOnePerLine("DriveType", fo->Drivetype);
  send.KeyMultipleStringsOnePerLine("Meta", fo->meta);
    ...
    bool verbose)
{
  ...
  if (include_items.size() > 0) {
    ...
    for (std::size_t i = 0; i < include_items.size(); i++) {
       ...
       * File = entries.
       */
      send.KeyMultipleStringsOnePerLine("File", incexe->name_list);

      /*
       * Plugin = entries.
       */
      send.KeyMultipleStringsOnePerLine("Plugin", incexe->plugin_list);
